document.addEventListener('DOMContentLoaded', () => {
    ...
    const blogThemeEl = document.getElementById('blog-theme');
    const apiKeyEl = document.getElementById('api-key');
    const resultArea = document.getElementById('result-area');

    generateBtn.addEventListener('click', async () => {
        const theme = blogThemeEl.value.trim();
        ...
        resultArea.textContent = 'AIが構成を考えています...';
        generateBtn.disabled = true;

        try {
            const prompt = `以下のテーマに関するブログ記事の構成案を作成してください。\n\n- 読者の興味を引くタイトル案を3つ\n- 導入、本文（3〜5つの見出し）、まとめ、という構成\n- それぞれの見出しで何を書くべきか簡単な説明\n\nテーマ: ${theme}`;
