document.addEventListener('DOMContentLoaded', () => {
    ...
    const productThemeEl = document.getElementById('product-theme');
    const apiKeyEl = document.getElementById('api-key');
    const resultArea = document.getElementById('result-area');

    generateBtn.addEventListener('click', async () => {
        const theme = productThemeEl.value.trim();
        const API_KEY = apiKeyEl.value.trim();

        if (!theme) {
            alert('テーマや特徴を入力してください。');
            return;
        }
        ...
        resultArea.textContent = 'AIが考えています...';
        generateBtn.disabled = true;

        try {
            const prompt = `以下のテーマに沿った、魅力的で短いキャッチコピーを5つ提案してください。\n\nテーマ: ${theme}`;
