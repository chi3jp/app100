document.addEventListener('DOMContentLoaded', () => {
    ...
    generateBtn.addEventListener('click', async () => {
        const description = serviceDescriptionEl.value.trim();
        const API_KEY = apiKeyEl.value.trim();

        if (!description) {
            alert('サービスまたは製品の概要を入力してください。');
            return;
        }
        ...
        resultArea.textContent = 'AIがFAQを生成しています...';
        generateBtn.disabled = true;

        try {
            const prompt = `以下のサービスまたは製品の概要に基づいて、よくある質問（FAQ）とその回答を5つ提案してください。\n\n概要: ${description}`;
